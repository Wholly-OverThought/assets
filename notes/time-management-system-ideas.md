...
### Blocks pages finished — no more "draw more if needed" gaps
Filled the remaining blank placeholder cards with two new universal blocks: **Home** (tidy a surface, dishes, laundry) and **Finance** (check balance, log spending, one bill review). Standard Blocks page: 8 named blocks (Hygiene, Body, Power Hour, Set Up Tomorrow, Skip, Home, Finance) + 1 genuine custom slot. Adaptive Blocks page mirrors this and adds **Rest** (scheduled rest/lie-down, ice/heat/elevate, screens off) as its own real block rather than leaving rest implicit — fits the tiered/capacity philosophy directly. Both pages still fit on one printed page each.

### Adaptive Edition promoted to a co-equal release
